...
def test_register_verification(client, db, django_user_model):
    ...
    db_user = django_user_model.objects.get(username=creds["username"])
    assert db_user
    ...
    key = re.search(r"confirm-email\/(.+)\/", verify_mail.body).group(1)
    assert key
    ...
    # Check verification
    verify_response = client.post("/users/registration/verify-email/", {"key": key})
