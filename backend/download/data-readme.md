...
The project proceeds along two complementary pathways. First, it has established an *index of Merovingian epistolary terminology* that is sensitive to all (textual and non-textual) aspects of lettercraft. This index can be consulted in the glossary provided on the Lettercraft website. We plan to include it in this dataset in the future.
...
Epistolary communication tends to perpetuate itself: letters trigger a response letter, gifts solicit a counter-gift, messengers delivering an oral message are sent back with a reply. Demarcating when one epistolary episode ends and another begins is thus a matter of interpretation. As a general rule, we have aimed to strike a balance between narrative coherence and analytical feasibility. Typically, a narrative story involving a letter and a response will be logged as one episode. However, if the chain of communication becomes too drawn out or complex, or if the response takes place much later in the story, we will split the communication chain over several episodes.
...
**Data collection** – The data was collected and annotated by hand. Each text lists the project members and student apprentices involved in the annotation process. Typically, a text would first be analysed using [Brepols’ full-text database search-tool](https://brepolis.net): annotators would use a working version of the Lettercraft glossary to search for occurrences of relevant epistolary terminology. After establishing a provisional list of relevant episodes for a text, annotators would read through the Latin texts in their entirety, aided by modern academic translations when available. This would result in a more precise and complete list of episodes, which would then be logged provisionally in the database. The database entries were composed by filling in a digital annotation form developed by the Centre for Digital Humanities at Utrecht University. No generative AI was used to fill in the database forms. Annotators would report on their work in weekly project meetings. This ensured consistency between annotators and offered a platform to discuss complex and dubious episodes. It also enabled the annotation rules to be finetuned as more data was processed. All entries were checked and edited by a senior project member before they were published.
