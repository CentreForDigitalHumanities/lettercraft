...
import { parseUserData, encodeUserData, setErrors, controlErrorMessages$, formErrorMessages$, updateFormValidity } from './utils';
...
describe('User utils', () => {
    ...
    describe('setErrors', () => {
        it('should set errors on associated controls', () => {
            ...
            expect(form.get('username')?.errors).toEqual({ 'invalid': 'Username is required.' });
            expect(form.get('email')?.errors).toEqual({ 'invalid': 'Email is invalid.' });
        });
    ...
    describe('controlErrorMessages$', () => {
        it('should return an Observable of error messages for a specific control', () => {
            const usernameMessages = TestBed.runInInjectionContext(() => toSignal(controlErrorMessages$(form, 'username')));
        ...
        it('should use errors based on a provided lookup name', () => {
            const usernameMessages = TestBed.runInInjectionContext(() => toSignal(
                controlErrorMessages$(form, 'username', 'email'))
            );
    ...
    describe('formErrorMessages$', () => {
        it('should return an Observable of error messages for the form', () => {
            const formMessages = TestBed.runInInjectionContext(() => toSignal(
                formErrorMessages$(form))
            );
    ...
    describe('updateFormValidity', () => {
        it('should update the validity of all controls in the form', () => {
            form.controls['username'].setErrors({ 'invalid': 'Username is required.' });
